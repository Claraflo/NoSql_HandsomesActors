...
if (flag == True):
	...
	options = Options()
	options.headless = True #Le navigateur travail sans ouvrir la fenetre
	...
	for line in lines :
		...
		#Recuperation de la photo de l'acteur, placee dans le dossier photos
		img = soup.find('img',{'id':"name-poster"})
		if img != None:
			urllib.request.urlretrieve(img.get('src'),'./photos/'+line.strip().replace(' ','_')+'.png')

		count += 1
		print("\nJ'ai fini de recuperer la liste de film. j'ai fait "+str(count)+" acteurs de la liste.\n")
		filmo.close()
